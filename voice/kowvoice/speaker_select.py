...
def current_name() -> str:
    ...
    return parse_conf(kowalski_conf_path()).get(CONF_KEY, "")
...
def run_loop(stdscr) -> int:
    ...
    from .mic_select import BAR_WIDTH, active_index, default_device_index, safe_addstr
    ...
    if not devices:
        safe_addstr(stdscr, 0, 2, "no output devices found — press a key")
        stdscr.timeout(-1)
        stdscr.getch()
        return 1

    cur = current_name()
    default_out = default_device_index(1)
    ...
    try:
        while True:
            ...
            stdscr.erase()
            safe_addstr(stdscr, 0, 2, "Select speaker / output", curses.A_BOLD)
            safe_addstr(stdscr, 1, 2,
                  "↑/↓ choose · hold t to test (volume ramps) · Enter/s save · q quit   (● = current)",
                  curses.A_DIM)
            active = active_index(devices, cur, default_out)
            ...
            safe_addstr(stdscr, row, 2, "volume " + "█" * filled + "·" * (BAR_WIDTH - filled))
            safe_addstr(stdscr, row + 1, 2, "hold t — the tone plays and gets louder", curses.A_DIM)
            if cur:
                cur_name = cur
            elif active >= 0:
