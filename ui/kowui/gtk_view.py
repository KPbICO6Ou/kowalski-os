...
class OmniWindow:
    ...
    def __init__(self, client: OmniClient):
        ...
        self.controller = OmniController(client, MarshalledCallbacks(self))
        ...
        allow.connect("clicked", self.on_confirm_clicked, True)
        deny.connect("clicked", self.on_confirm_clicked, False)
    ...
    def on_activate(self, _entry) -> None:
        prompt = self.entry.get_text().strip()
        if not prompt:
            return
        self.entry.set_text("")
        self.append_text(f"\n> {prompt}\n")
        self.async_thread.submit(self.controller.submit(prompt))
    ...
    def on_confirm_clicked(self, _button, approved: bool) -> None:
        ...
        self.async_thread.submit(self.controller.answer_confirm(request_id, approved))
